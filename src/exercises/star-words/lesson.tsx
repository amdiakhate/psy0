 ...
 *   A = ORIGINE  B = CABINET  C = NOURRIR  D = FERMIER  E = CANTINE  F = COURBER
 * Cases communes : A/D = I · A/E = I · B/E = N · B/F = B · C/F = U · C/D = R
 */
const WORDS = [
  'CONFIER',
  'SOURIRE',
...
const RARE: P = [null, 'CABINET', null, null, null, 'COURBER'];
const PROPAGATE: P = [null, 'CABINET', 'NOURRIR', null, null, 'COURBER'];
const CONFLICT: P = ['CONFIER', 'CABINET', 'NOURRIR', 'FERMIER', null, 'COURBER'];
const FULL: P = ['ORIGINE', 'CABINET', 'NOURRIR', 'FERMIER', 'CANTINE', 'COURBER'];
...
export const lesson: Lesson = {
  ...
  steps: [
    ...
    },
    {
      ...
      observe:
        'Parcours les couples : (N,I) (U,I) (R,I) (N,I) (U,B) (U,R) (I,I) (B,N) (N,I). Le N, le I et le U reviennent souvent. Le B, lui, n’apparaît que deux fois : en 3e lettre de CABINET et en 5e lettre de COURBER.',
    ...
    },
    {
      ...
      observe:
        'CABINET en B et COURBER en F : leur case commune (entourée de cyan) porte le B, et une seule lettre y est écrite. Deux emplacements verrouillés.',
    ...
    },
    {
      ...
      observe:
        'COURBER a un U en 3e lettre. Sa deuxième case commune (avec l’emplacement C) impose donc un U en 3e lettre du mot voisin : NOURRIR convient.',
    ...
    },
    {
      ...
      observe:
        'CONFIER posé en A. Sa case commune avec FERMIER devient rouge et affiche N/I : les deux mots exigent deux lettres différentes sur la MÊME case.',
      why: 'L’étoile fait la vérification à ta place. Tu n’as jamais besoin de recompter des lettres : une case rouge = mot à retirer. CONFIER a (N,I) là où il faudrait (I,·) — c’est un distracteur.',
      pitfall:
      ...
      action: 'Retire CONFIER et cherche un mot dont la 3e lettre est un I : ORIGINE (I,I).',
    },
    {
      scene: 'full',
      title: 'Refermer et valider — sans chercher « la » solution',
      observe:
        'Les 6 cases communes sont bleues et cohérentes : I, I, N, B, U, R. Il reste CONFIER, SOURIRE et JANVIER — les trois distracteurs.',
      why:
        'La consigne officielle le dit : la correction proposée n’est qu’UNE solution parmi d’autres. Le validateur contrôle les croisements, pas une liste mémorisée. S’il n’y a pas d’erreur, le point est pour toi.',
      action:
