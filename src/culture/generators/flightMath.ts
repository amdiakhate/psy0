...
type FlightMathDrillType = Extract<CultureDrillType, 'distance' | 'time' | 'speed'>;
...
function base(id: string, drillType: FlightMathDrillType, question: string, answer: number, explanation: string): CultureQuestion {
  return {
    id,
    tier: 'core',
...
export function generateFlightMathQuestion(rng: Rng, requestedType?: FlightMathDrillType): CultureQuestion {
  const kinds: FlightMathDrillType[] = ['distance', 'time', 'speed'];
  ...
  if (kind === 'distance') {
    return base(`generated-distance-${nonce}`, kind, `À ${speed} kt pendant ${minutes} min, quelle distance parcours-tu en NM ?`, distance, `${minutes} min = ${f.label} h. D = V × T = ${speed} × ${f.label} = ${distance} NM.`);
  }
  if (kind === 'time') {
    return base(`generated-time-${nonce}`, kind, `Tu dois parcourir ${distance} NM à ${speed} kt. Combien de minutes faut-il ?`, minutes, `T = D ÷ V = ${distance} ÷ ${speed} h = ${f.label} h, soit ${minutes} minutes.`);
  }
  return base(`generated-speed-${nonce}`, kind, `Quelle vitesse en kt faut-il pour parcourir ${distance} NM en ${minutes} min ?`, speed, `${minutes} min = ${f.label} h. V = D ÷ T = ${distance} ÷ ${f.label} = ${speed} kt.`);
}
